...
![](/img/guide/api/siem-integration/saviynt-eic_siem-integration_reference-architecture_SA77.png)
...
Create a new runtime analytics control (V2) using an SQL query. For more information, see Creating Elasticsearch-based Analytics Controls (Version 2). While creating an analytics control, copy the following query in the Analytics Query parameter.
...
1. Create a user, for example, siem-sid. For more information, see Creating Users.
1. Change the password for the user. For more information, see Administrator Functions.
1. Create a SAV role, for example, ROLE_SIEM.
1. Assign permissions to the newly created SAV role.
    1. Assign the permission to access the web service URL of the Saviynt fetchRuntimeControlsDataV2 API. For more information, see Understanding the SAV Role Parameters.
    1. Assign the permission to verify the analytics record that you created. For more information, see Understanding the SAV Role Parameters.
1. Associate the SAV role with the user created in Step1. For more information, see Understanding the SAV Role Parameters.
...
1. Ensure that you add the following in the body of the API:
...
1. **Click Create**. The SAV Role details are displayed as shown below:
1. Click **Feature Access**.
1. Click **Actions** and select the **Home** access.
1. Click **Web Service Access**x    .
1. Click **Actions** and select the following accesses:
...
You must add only these accesses, other accesses are added by default as shown below.
